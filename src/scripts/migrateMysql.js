...
const fs = require('fs');
const connectMongo = require('../config/connectDatabase');

(async () => {
  await connectMongo();

  // === Run all Migrations ===
  const migrationPath = path.join(__dirname, '../migrations/mysql');
  ...
  // === Run all Seeders ===
  const seederPath = path.join(__dirname, '../seeders/mysql');
